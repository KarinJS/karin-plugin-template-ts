import { common as Common } from '@template/lib/imports'
import { segment, Plugin, Bot, common } from 'node-karin'
...
export class sendMsg extends Plugin {
  ...
  async sendForward () {
    const makeForward = common.makeForward([
      segment.text('这是一条转发消息~'),
    ], this.e.user_id, this.e.sender.nick)
    this.replyForward(makeForward)
  }
}
